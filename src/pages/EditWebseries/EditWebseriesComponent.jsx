...
const EditWebseriesComponent = () => {
  ...
  const videoData = async () => {
    try {
      const result = await axios.get(
        `${process.env.REACT_APP_LOCALHOST_URL}/php/API/fetch_single_webseries?webseries_uuid=${param.id}`
      );
      result.data.map((item) => {
        if (item.video_uuid === param.id) {
          setVideoTitle(item.name);
          setVideoCategory(item.web_series_uuid);
          setDescription(item.video_desc);
          setImageFileName(item.name + ".jpg");
          setVideoFileName(item.name + ".mp4");
          setVideoUploading(false);
          setImageFile(item.thumbnail_ipfs);
          setVideoFile(item.video_uid);
        }
      });
    } catch (error) {
      console.log(error);
  ...
  async function handleFileUpload(videoId) {
    ...
    async function imageUpload() {
      ...
      try {
        ...
        var config = {
          method: "post",
          url: `${process.env.REACT_APP_LOCALHOST_URL}/php/API/update_webseries`,
          data: data,
        };
        if (count === 1) {
          await axios(config)
  ...
  async function fetchData() {
    try {
      const result = await axios.get(
        `${process.env.REACT_APP_LOCALHOST_URL}/php/API/fetch_webseries_category.php`
      );
          ...
          <div className="mb-5" id="select">
            ...
            >
              ...
              {fetchCategory.map((category) => (
                <option
                  value={category.web_series_uuid}
                  key={category.web_series_uuid}
                >
                  {category.name_of_web_series}
                </option>
              ))}
            </select>
          </div>
...
export default EditWebseriesComponent;
